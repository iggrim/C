...
int main(void)
{
    ...
    while (s_gets(file_src, SLEN) && file_src[0] != '\0') // file_src - адрес (типа char *), где должны сохраняться введенные данные
    {
        ...
        else if ((fs = fopen(file_src, "r")) == NULL) //открываем файл file_src(адрес типа char*)
            fprintf(stderr, "Can't open %s\n", file_src);
        else
        {
...
{
    size_t bytes;
    static double temp[1]; // allocate once // выделить память один раз // массив изодного элемента размером 8 байт(double)
    ...
    while ((bytes = fread(temp,sizeof(double),1,source)) > 0){
        fwrite(temp, sizeof (double), bytes, dest);
