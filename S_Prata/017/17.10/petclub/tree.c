...
*/  // SeekItem возвращает структуру Pair
    if (SeekItem(pi, ptree).child != NULL) // pi - структура типа Item(элемент - имя, вид), уже заполнена введеннми данными
    {
    ...
    else // попробовать сразу установить адрес ptree->root в адрес look.child->left или Right возвращаемого узла функцией SeekItem
        AddNode(new_node,ptree->root); /* добавить новый узел в дерево, адрес  */

//if (look.child->left != NULL)
//    ptree->root = look.child->left
//else
//    ptree->root = look.child->right
...
static void AddNode (Trnode * new_node, Trnode * root)//новая нода и корневая нода
{
    if (ToLeft(&new_node->item, &root->item)) //новая нода и корневая нода
    {
        if (root->left == NULL)      /* empty subtree       */
            ...
            AddNode(new_node, root->left);/* рекурсивный вызов AddNode продвижение по дереву с корневого узла, по адресам узлов*/
    }
...
static Pair SeekItem(const Item * pi, const Tree * ptree) //pi источник ввода - ptree принимающая структура(дерево)
{
    ...
    {   // значение look.child->item - адрес (Item *) 0x885b10 как и у look.child
                                             ...
                                             // ToLeft возращает либо nrue(если значение petname pi меньше petname item ) либо false в обратном случае
        {
            ...
            look.child = look.child->left; // продвижение в цикле поиска влево начиная с уорня(работаем с адресом значения слева look.child->left)
        }
